...
class warn(minqlx.Plugin):
    ...
    def handle_player_connect(self, player):
        ...
        if warned:
            ...
            if strike >= self.get_cvar("qlx_maxStrikes", int):
                return "Você foi banido por alguma razao e foi alertado: {}: AVISADO {} VEZES.".format(reason, strike)

    def handle_player_loaded(self, player):
        warned = self.is_warned(player.steam_id)
        if warned:
            strike, reason, expires = warned
            self.msg("^1ATENCAO^7! {} conectou. Alertado por: ^6{}^7, strike: ^6{}^7/^6{}^7, expira: ^6{}^7.".format(player.name, reason, strike, self.get_cvar("qlx_maxStrikes", int), expires))
    ...
    def cmd_warn(self, player, msg, channel):
        ...
        if strike + 1 < self.get_cvar("qlx_maxStrikes", int):
            self.msg("{} FOI AVISADO PRA PARAR CA PALHACADA! for: ^6{}^7, strike: ^6{}^7/^6{}^7, expires: ^6{}^7.".format(name, reason, strike + 1, self.get_cvar("qlx_maxStrikes", int), expires))
        elif strike + 1 >= self.get_cvar("qlx_maxStrikes", int):
            try:
                self.kick(ident, "FOI BANIDO PQ NAO PAROU COM A PALHACADA: {}: AVISOS {} VEZES.".format(reason, strike + 1))
            except ValueError:
                self.msg("^6{} ^7FOI AVISADO POR FAZER PALHACADA: ^6{}^7: AVISOS ^6{} ^7VEZES.".format(name, strike + 1))
